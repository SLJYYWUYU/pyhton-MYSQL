...
class menu:
    ...
    @audit_log(user_role="老师查询学生成绩",action="tea_grade")
    def tea_grade(self):
        self.course.place_forget()
...
class tcGrade:
    def __init__(self, frame, teacherID):
        ...
        self.studentTable = ttk.Treeview(self.frame,
                                        ...
                                        show='headings')
        # 成绩统计信息标签
        self.statsLabel = tkinter.Label(self.frame, text='平均分：  最高分：  最低分：')
    ...
    def initialize(self):
        ...
        self.statsLabel.place(x=self.courseTable.winfo_x() + self.courseTable.winfo_width() + 5,
                      y=self.courseTable.winfo_y() + self.courseTable.winfo_height() - 90,
                      anchor='sw')
    ...
    # 获取成绩统计
        grades = [float(x[2]) for x in info if x[2] is not None]
        if grades:
           avg = sum(grades) / len(grades)
           maximum = max(grades)
           minimum = min(grades)
           self.statsLabel.configure(text=f'平均分：{avg:.1f}   最高分：{maximum:.1f}   最低分：{minimum:.1f}')
        else:
           self.statsLabel.configure(text='平均分：-   最高分：-   最低分：-')     
